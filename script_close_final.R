############ SPECIAL INSTRUCTIONS : DON'T RUN ALL THE FILE IN ONE TIME #######
############ YOU SHOULD NOT RUN THE TWO LAST LINES UNTIL YOU FINISHED READING THE RESULTS OF THE REST OF THE CODE ######

# This file is essentialy the same as script_far_final but in this file we make analysis of closer clusters compared to the other file.
...
mu <- list(c(-1, -1), c(1, -1), c(-1, 1), c(1, 1)) # Make clusters closer than in the other example
...
saveRDS(dp, file = "dp_model_close_good.rds")
...
# missclassification
print(sum(as.character(df$true_cluster) != as.character(df$vi_cluster)))
...
#summary(credible_ball)  # very long print in the console but the results are still readable at the end
# We comment the line just above, we rewrite at the end of the script and you should run it at the end
# Otherwise it may become unreadable in the console.
...
#summary(credible_ball_binder)  # Very long output in the console but the results at the end are still readable
# We comment the line just above, we rewrite at the end of the script and you should run it at the end
# Otherwise it may become unreadable in the console.
...
######## DANGER DON'T RUN THIS IF YOU HAVE NOT FINISHED READING THE OTHER RESULTS BECAUSE IT WILL MAKE A
######## VERY LONG PRINT IN THE CONSOLE
# It will make very long prints in the console but the results at the end are still readable.

summary(credible_ball)  # Don't run both at the same time


summary(credible_ball_binder) # Don't run both at the same time